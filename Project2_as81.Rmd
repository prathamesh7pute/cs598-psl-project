...
After performing the 10 fold validation and selecting the best lambda we retrained the model with the selected best lambda and family binomial and type.measure class and used this model for predicting class on the test data. We got around 64% accuracy for the test data.
...
```{r echo=FALSE, results='asis'}
  ...
  cat(c("\\begin{table}[!htb]
    ...
    \\begin{minipage}{.5\\linewidth}
      \\centering
        \\caption{Lasso Confusion Matrix on complete data}",
        t2,
    "\\end{minipage} 
    \\end{table}"
    ))  
...
```{r echo=FALSE, results='asis'}
  ...
  cat(c("\\begin{table}[!htb]
    ...
    \\begin{minipage}{.5\\linewidth}
      \\centering
        \\caption{SVM Confusion Matrix on complete data}",
        t2,
    "\\end{minipage} 
    \\end{table}"
    ))  
...
```{r echo=FALSE, results='asis'}
  ...
  cat(c("\\begin{table}[!htb]
    ...
    \\begin{minipage}{.5\\linewidth}
      \\centering
        \\caption{RandomForest Confusion Matrix on complete data}",
        t2,
    "\\end{minipage} 
    \\end{table}"
    ))  
...
Please check the Reference below in the end added with abstact summary.
...
We then decided to use the random forest as we have use it in Q1 with PCA and compare performance.
