...
```{r fig.height = 2, fig.width = 10, out.width = '100%', fig.align = "center", echo = FALSE}
...
```{r lasso_results, echo=FALSE, results='asis'}
  kable(lasso_results, "latex", caption = "Lasso Results", booktabs = T) %>%
...
```{r include = TRUE}
  ...
  temp <- confusionMatrix(as.factor(svmPred), as.factor(all_data_y))
  kable(temp$table, caption="Linear SVM Confusion Matrix")
```
...
```{r include = TRUE}
#plot(ksvm_model, train.data, PC1 ~ PC3)
```


```{r include=TRUE}
  ksvmPred <- predict(ksvm_model, all_data_x)
  temp <- confusionMatrix(as.factor(ksvmPred), as.factor(all_data_y))
  kable(temp$table, caption="RBFDOT NonLinear SVM Confusion Matrix")
```
...
```{r}
  extract_features <- function(img_in) {
    ...
    img_th = EBImage::combine(mapply(function(frame, th) frame > th, getFrames(img_in_th), threshold, SIMPLIFY = FALSE))
    img_th_val <- img_th
...
```{r fig.height = 4, fig.width = 10, out.width = '100%', fig.align = "center", echo = FALSE}
  ...
  p6 <- cv_glmnet_model$glmnet.fit %>%
    ...
    labs(x = "",
         ...
         title = "Plot 9: What features are important in prediction")
